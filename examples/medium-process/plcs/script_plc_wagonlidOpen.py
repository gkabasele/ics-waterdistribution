...
if os.path.exists("wagonlidOpen.log"):
    os.remove("wagonlidOpen.log")

logging.basicConfig(filename = "plcs_log" + "/wagonlidOpen.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-wagonlidOpen", wagonlidOpen = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("wagonlidOpen", args.period, args.duration)
    plc.wait_end(True)
