...
print "Launching silo1"
if os.path.exists("silo1.log"):
    os.remove("silo1.log")

logging.basicConfig(filename = "plcs_log" + "/silo1.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-silo1", silo1 = ('h', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("silo1", args.period, args.duration)
    plc.wait_end(True)
