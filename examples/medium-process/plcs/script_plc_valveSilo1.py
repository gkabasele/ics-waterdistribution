...
print "Launching valveSilo1"
if os.path.exists("valveSilo1.log"):
    os.remove("valveSilo1.log")

logging.basicConfig(filename = "plcs_log" + "/valveSilo1.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-valveSilo1", valveSilo1 = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("valveSilo1", args.period, args.duration)
    plc.wait_end(True)
