...
print "Launching approvisioning2"
if os.path.exists("approvisioning2.log"):
    os.remove("approvisioning2.log")

logging.basicConfig(filename = "plcs_log" + "/approvisioning2.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-approvisioning2", approvisioning2 = ('h', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("approvisioning2", args.period, args.duration)
    plc.wait_end(True)
