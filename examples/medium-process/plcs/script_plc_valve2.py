...
print "Launching valve2"
if os.path.exists("valve2.log"):
    os.remove("valve2.log")

logging.basicConfig(filename = "plcs_log" + "/valve2.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-valve2", valve2 = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("valve2", args.period, args.duration)
    plc.wait_end(True)
