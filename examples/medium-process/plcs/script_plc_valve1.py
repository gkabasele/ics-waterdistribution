...
if os.path.exists("valve1.log"):
    os.remove("valve1.log")

logging.basicConfig(filename = "plcs_log" + "/valve1.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)
