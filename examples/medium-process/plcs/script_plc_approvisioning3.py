...
print "Launching approvisioning3"

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-approvisioning3", approvisioning3 = ('h', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("approvisioning3", args.period, args.duration)
    plc.wait_end(True)
