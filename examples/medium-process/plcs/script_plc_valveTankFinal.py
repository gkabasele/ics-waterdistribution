...
print "Launching valveTankFinal"
if os.path.exists("valveTankFinal.log"):
    os.remove("valveTankFinal.log")

logging.basicConfig(filename = "plcs_log" + "/valveTankFinal.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-valveTankFinal", valveTankFinal = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("valveTankFinal", args.period, args.duration)
    plc.wait_end(True)
