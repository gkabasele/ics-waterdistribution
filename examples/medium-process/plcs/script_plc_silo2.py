...
print "Launching silo2"
if os.path.exists("silo2.log"):
    os.remove("silo2.log")

logging.basicConfig(filename = "plcs_log" + "/silo2.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-silo2", silo2 = ('h', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("silo2", args.period, args.duration)
    plc.wait_end(True)
