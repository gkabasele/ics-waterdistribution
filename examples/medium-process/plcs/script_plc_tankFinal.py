...
if os.path.exists("tankFinal.log"):
    os.remove("tankFinal.log")

logging.basicConfig(filename = "plcs_log" + "/tankFinal.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)
