...
print "Launching wagonStart"
if os.path.exists("wagonStart.log"):
    os.remove("wagonStart.log")

logging.basicConfig(filename = "plcs_log" + "/wagonStart.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-wagonStart", wagonStart = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("wagonStart", args.period, args.duration)
    plc.wait_end(True)
