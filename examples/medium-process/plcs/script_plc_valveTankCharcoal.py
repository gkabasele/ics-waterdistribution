...
print "Launching valveTankCharcoal"
if os.path.exists("valveTankCharcoal.log"):
    os.remove("valveTankCharcoal.log")

logging.basicConfig(filename = "plcs_log" + "/valveTankCharcoal.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-valveTankCharcoal", valveTankCharcoal = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("valveTankCharcoal", args.period, args.duration)
    plc.wait_end(True)
