...
print "Launching tankCharcoal"
if os.path.exists("tankCharcoal.log"):
    os.remove("tankCharcoal.log")

logging.basicConfig(filename = "plcs_log" + "/tankCharcoal.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-tankCharcoal", tankCharcoal = ('h', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("tankCharcoal", args.period, args.duration)
    plc.wait_end(True)
