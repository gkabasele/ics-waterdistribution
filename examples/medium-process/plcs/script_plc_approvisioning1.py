...
print "Launching approvisioning1"
if os.path.exists("approvisioning1.log"):
    os.remove("approvisioning1.log")

logging.basicConfig(filename = "plcs_log" + "/approvisioning1.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-approvisioning1", approvisioning1 = ('h', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("approvisioning1", args.period, args.duration)
    plc.wait_end(True)
