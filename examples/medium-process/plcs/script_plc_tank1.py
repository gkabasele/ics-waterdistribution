...
if os.path.exists("tank1.log"):
    os.remove("tank1.log")

logging.basicConfig(filename = "plcs_log" + "/tank1.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-tank1", tank1 = ('h', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("tank1", args.period, args.duration)
    plc.wait_end(True)
