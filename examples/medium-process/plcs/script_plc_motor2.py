...
print "Launching motor2"
if os.path.exists("motor2.log"):
    os.remove("motor2.log")

logging.basicConfig(filename = "plcs_log" + "/motor2.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-motor2", motor2 = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("motor2", args.period, args.duration)
    plc.wait_end(True)
