...
print "Launching wagonEnd"
if os.path.exists("wagonEnd.log"):
    os.remove("wagonEnd.log")

logging.basicConfig(filename = "plcs_log" + "/wagonEnd.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-wagonEnd", wagonEnd = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("wagonEnd", args.period, args.duration)
    plc.wait_end(True)
