...
print "Launching valveTank1"
if os.path.exists("valveTank1.log"):
    os.remove("valveTank1.log")

logging.basicConfig(filename = "plcs_log" + "/valveTank1.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-valveTank1", valveTank1 = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("valveTank1", args.period, args.duration)
    plc.wait_end(True)
