...
print "Launching valveSilo2"
if os.path.exists("valveSilo2.log"):
    os.remove("valveSilo2.log")

logging.basicConfig(filename = "plcs_log" + "/valveSilo2.log", mode = 'w', format='[%(asctime)s][%(levelname)s][%(pathname)s-%(lineno)d] %(message)s', level = logging.INFO)

def main(args):
    plc = PLC(args.ip, args.port, args.store, "plc-valveSilo2", valveSilo2 = ('c', 1))
    if args.create_ex:
        plc.export_variables(args.filename)
    plc.run("valveSilo2", args.period, args.duration)
    plc.wait_end(True)
